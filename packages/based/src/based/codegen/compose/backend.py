...
def _to_colors(nodes: dict[str, ColorNode]) -> dict[str, Color]:
    return {k: _to_color(v) for k, v in nodes.items()}
...
class Compose(Backend):
    ...
    def codegen(self, target: TargetNode, artifact: ArtifactNode) -> dict[str, bytes]:
        ...
        light_colors = _to_colors(get_variant("light"))
        dark_colors = _to_colors(get_variant("dark"))
